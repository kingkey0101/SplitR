...
export const spendingInsights = inngest.createFunction(
  ...
  { cron: "0 8 1 * * " }, //1st of every month @ 8am
  async ({ step }) => {
    ...
    //iterate users & send insight email
    const reuslts = [];

    for (const user of users) {
      //pull last month expenses(skip if none)
      const expense = await step.run(`Expenses • ${user._id}`, () =>
        convex.query(api.inngest.getUserMonthlyExpenses, { userId: user._id })
      );
      if (!expense?.length) continue;

      const expenseData = JSON.stringify({
        expense,
        totalSpent: expense.reduce((sum, e) => sum + e.amount, 0),
        categories: expense.reduce((cats, e) => {
      ...
      //prompt + AI call using step.ai.wrap (retry-aware)
      const prompt = `
...
5. Recommendations for Next Month
      ...
      try {
        ...
        reuslts.push({ userId: user._id, succes: true });
      } catch (error) {
        reuslts.push({
          userId: user._id,
          success: false,
    ...
    return {
      processed: reuslts.length,
      success: reuslts.filter((r) => r.succes).length,
